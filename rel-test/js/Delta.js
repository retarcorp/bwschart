...
Delta.prototype = {
    ...
    render: function(f) { 
        ...
        if (App.supervisor && !this.supervisorEnable && Data.lastABI) {
            
            console.log(this.supervisorEnable);
            ...
            if (this.CurType !== "ordertape") {
                ...
                this.transform({
                    new_start_ox: (Data.lastDelta.index * gs.FIXED_TIME_STEP + gs.WIDTH/2) * gs.OX_SCALE,
    ...
    buildSprites: function (candles) {
       ...
       sprites.forEach(function(candle){
        ...
        //console.log('Столбики отрисованы. Время: ', new Date(), '\n Задержка: ', GlobalTime.getTime() - new Date().getTime(), '\n');
        ...
        //console.log('Линии конца дна/месяца созданы. Время: ', new Date(), '\n Задержка: ', GlobalTime.getTime() - new Date().getTime(), '\n');
                        ...
                        switch(elem.r_type) {
                            case 'left': 
                                if (elem.f_x < elem.s_x) {
                                    elem.f_x = this.cordForVerticalLine;
                                }
                                else {
                                    elem.s_x = this.cordForVerticalLine;
                                }
                                break;
        ...
        if (cordY > 0 && cordX > 0) {//!!~~
            ...
            if(!this.mouseFlag || e !== undefined){
                if(cordX > minX && cordX < maxX){
                    if (App.CurrentGraph.IsZoomBorderCrossing || gs.IS_TIME_ZOOM_BORDER) adder = 0;
                        if(App.CurrentGraph.CurType !== 'ordertape'){
    ...
    onDragged: function (x, y, supervisor) {
        //создание объекта, хранящего параметры трансформации графикa
...
CandleSpriteDelta.prototype = {
    ...
    , render: function () {
        ...
        var green = App.CurrentGraph.visualSettings.DIR_PLUS,              //зеленый цвет тика
            ...
            gray = App.CurrentGraph.visualSettings.DIR_0,
        ...
        //     //  console.log('startpricecandle = ' + this.Candle.start_price, this.Candle.startTs);
    ...
    , getCandleByIndex: function(index){
        if(App.CurrentGraph.CurType !== 'ordertape'){
            ...
            if(Candle === undefined){
                ...
                // console.log('id = ' + (Candle.id - App.CurrentGraph.FirstIdOrder),Candle.volume, this.tsToTime(Candle.secs*1000))
