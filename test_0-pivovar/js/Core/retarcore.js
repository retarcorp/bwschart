		...
		,Templates: {
		    ...
		    ,apply: function(tpl, data){
		        ...
		        for (var i in data){
    		        ...
    		        for (var j in data[i]){
    		            try{
    		                // For some reason, there were a replacement of double quote to single quote. As soon as I don't remember,why was it done, it is commented and replaced.
    		                //tp = tp.split("{{"+j+"}}").join(data[i][j].toString().replace(/"/gi,"'"));
    		                tp = tp.split("{{"+j+"}}").join(data[i][j].toString());
