...
Graph.prototype = {
    ...
    createVerticalLine: function(e) {
        ...
        if(e.type === "mousedown"  && App.currentPlatform === 'PC'){
            ...
            //if (App.CurrentGraph.IsZoomBorderCrossing && gs.IS_TIME_ZOOM_BORDER) newSprite.x = 0.75 + this.cordForVerticalLine;
            this.sprites.push(newSprite);
            this.spritesEdditor = false;
            Ui.addHelpBar('', false);
        }
    ...
    deleteMarketProfile: function(id) {
...
//        console.log('bom');
    ...
    buildData: function (f, candles, isFullIntervalInCache) {
        ...
        if(isFullIntervalInCache){
            ...
            this.buildSprites(candles, true);
        }else{
            if(candles.length!==0){
    ...
    buildSprites: function (candles, isCandles) {
        ...
        if(this.sprites.length !== undefined){
...
//            if (candle.Candle && candle.Candle.volume == 0) console.log(candle.Candle); 
    ...
    drawXGridMarks: function () {
        ...
        while (currentW < gs.WIDTH - right_indent) {
            ...
            console.log(new Date(currentGridMarkTime).getDay());
            if (new Date(currentGridMarkTime).getDay() < 5 && new Date(currentGridMarkTime).getDay() > 0) {
                if(gs.TIMEFRAME === 'D1'){
    ...
    onDown: function (e) {
        ...
        if(e.type === 'mousedown'  && App.currentPlatform === 'PC'){
            ...
            if (App.utill.fetchCandle) App.utill.findCandle('footprint');
            ...
            if ((x <= gs.WIDTH - right_indent) && (y <= gs.HEIGHT - bottom_indent)) {
                if(this.rectangleEdditor){
                    ...
                    console.log(this.xDashLineSprite.x);
...
FootPrintGraph.prototype.getSprites = function (f) {
    ...
    if(!Graph.requestInProcess){
        ...
        Data.getCandlesForInterval({
            ...
            f: f,
            holidays: 2*this.holidays,
            isHolidaysCome: this.isHolidaysCome
        });
    }
};
            ...
            // if(!gs.IS_TIME_ZOOM_BORDER){
                ...
                if(array_of_ticks.length != 0){
                    ...
                    for(var i = firstPos; i <= lastPos; i++){
                        ...
                        if (gs.ZOOM_Y > gs.MIN_Y_ZOOM * 0.8) {
...
FootprintGraphSettings.prototype = {
    ...
    , tsToTime: function (ts) {
        ...
        if (Math.ceil(seconds/10.0)*10 >= 60) {
            seconds = Math.ceil(seconds/10.0)*10 - 60;
            minutes++;
            if (minutes >= 60){
                hours++;
                minutes -= 60;
        ...
        if (this.IS_TIME_ZOOM_BORDER) {
            if (Math.ceil(seconds/10.0)*10 >=30) {
                seconds = Math.ceil(seconds/10.0)*10 - 30;
                minutes++;
                if (minutes >= 60){
                    hours++;
                    minutes -= 60;
