_.cImager = {
	...
	,init: function(input){
		...
		$window.find("input[type='file']").change(function(){
			...
			_.new("div").fileUpload(e.attr("name"),
			_.cImager.Data.Modules.uploadImage,{
			        folder:$window.find(".cImager_content").data("folder")
			        ,img: $window.data("image")
		...
		$window.find("img.cImager_main").click(function(){
			...
			$window.find(".cImager_window .cImager_top select").val = $window.data("folder");
